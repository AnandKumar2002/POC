...
import { Button, Card, CardContent, TextField, Typography, Box, Modal } from '@mui/material';
...
const ButtonOutlined = styled(Button)<{ selected?: boolean }>(({ selected }) => ({
  color: selected ? '#fff' : '#1976d2',
  backgroundColor: selected ? '#1976d2' : 'transparent',
  borderRadius: '6px',
  '&:hover': {
    backgroundColor: selected ? '#1976d2' : 'rgba(25, 118, 210, 0.1)',
    color: selected ? '#fff' : '#1976d2',
  },
}));

const CustomModal: React.FC<{ open: boolean, onClose: () => void, title: string, message: any}> = ({ open, onClose, title, message }) => {
  return (
...
const DownloadReceipt: React.FC = () => {
  ...
  const handleDownloadReceipt = async () => {
    if (mobile.length === 10 && receiptType === 'tent') {
      try {
        const response = await axios.post(`${baseUrl}/download-receipt`, {
          mobile,
          receiptType: 'tent',
        });
        ...
        if (response.data && response.data.data && response.data.data.download_url) {
          window.open(response.data.data.download_url, '_blank');
        } else if(response.data && response.data.success && response.data.message === "failed payment") {
  ...
  return (
    <CenteredContainer>
      <StyledCard>
        <CardContent>
          <Typography variant="h5" component="div" gutterBottom sx={{ mb: 4, fontWeight: 'bold', color: '#1976d2' }}>
            Download Receipt
          </Typography>
          <Box sx={{ mb: 2 }}>
            <ButtonOutlined
            ...
            <>
              ...
              <Box component="form" noValidate autoComplete="off" sx={{ mt: 1 }}>
                ...
                <Button
                  ...
                  sx={{
                    borderRadius: '6px',
                    backgroundColor: isButtonDisabled ? '#cccccc' : '#1976d2',
                    '&:hover': {
                      backgroundColor: isButtonDisabled ? '#cccccc' : '#1565c0',
                    },
                  }}
                >
